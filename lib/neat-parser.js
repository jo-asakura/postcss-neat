...
var atRules = {
  'fill-parent': function fillParent() {
    return _grid2['default'].fillParent();
  },
  'omega': function omega(query, direction) {
    return _grid2['default'].omega(query, direction);
  },
  'outer-container': function outerContainer(maxWidth) {
    return _grid2['default'].outerContainer(maxWidth);
  },
  'pad': function pad() {
    ...
    return _grid2['default'].pad(padding);
  },
  'row': function row(display) {
    return _grid2['default'].row(display);
  },
  'shift': function shift(columns, containerColumns, direction) {
    return _grid2['default'].shift(columns, containerColumns, direction);
  },
  'span-columns': function spanColumns(columns, containerColumns, display, direction) {
    return _grid2['default'].spanColumns(columns, containerColumns, display, direction);
  }
};
